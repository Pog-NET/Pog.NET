...
impl StackTypes {
    pub fn new(selected: Types, i64type: Option<i64>, strtype: Option<String>) -> Self {
        Self {
            selected,
            i64type,
            strtype,
...
fn parse_args(args: Split<&str>, stack: &mut Vec<StackTypes>) -> Vec<Arg> {
    ...
    for i in args {
        ...
        if strpush {
            if i == "\"" {
                types.push(Arg {
                    selected: ArgTypes::String,
                    register: None,
                    int: None,
                    label: None,
                    string: Some(strbuf[0..strbuf.len() - 1].to_string()),
                });
                strpush = false;
            } else {
        ...
        if i.starts_with("#") {
            types.push(Arg {
                selected: ArgTypes::Int,
                register: None,
                int: Some(i.replace("#", "").parse().unwrap()),
                label: None,
                string: None,
            })
        } else if i.starts_with("$") {
            types.push(Arg {
                selected: ArgTypes::Register,
                register: Some(i.replace("$", "")),
                int: None,
                label: None,
                string: None,
            })
    ...
    while line_num < prog_len - 1 {
        ...
        if op == "push" {
            ...
            if args.get(0).unwrap().selected == ArgTypes::Int {
                ...
                stack.push(StackTypes::newstr(
                    args.get(0).unwrap().string.clone().unwrap(),
                ));
            } else if args.get(0).unwrap().selected == ArgTypes::Register {
                stack.push(
                    get_register(
                        args.get(0).unwrap().register.clone().unwrap(),
                        registers.clone(),
                    )
                    .unwrap(),
                )
                ...
                panic!(
                    "Invalid type for push: {:#?} at line {}",
                    args.get(0).unwrap().selected,
                    line_num + 1
                )
            }
        } else if op == "add" {
            let a: StackTypes = stack
                .pop()
                .expect(format!("Stack underflow at line {}", line_num + 1).as_str());
            let b: StackTypes = stack
                .pop()
                .expect(format!("Stack underflow at line {}", line_num + 1).as_str());
            if a.selected != Types::I64Type {
                panic!(
                    "Invalid type for add: {:#?} at line {}",
                    a.selected,
                    line_num + 1
                )
            }
            ...
            if labels.contains_key(&args.get(0).unwrap().clone().label.unwrap()) {
                ...
                panic!(
                    "Label: {} doesnt exist at line {}",
                    &args.get(0).unwrap().clone().label.unwrap(),
                    line_num + 1
                )
            }
        } else if op == "je" {
            ...
            if args.clone().get(0).unwrap().selected != ArgTypes::Register {
                panic!(
                    "Invalid type for pop: {:#?} at line {}",
                    args.clone().get(0).unwrap().selected,
                    line_num + 1
                )
            }
            registers = set_register(
                args.get(0).unwrap().clone().register.unwrap(),
                stack.pop().unwrap(),
                registers,
            )
        } else if op == "nout" {
            ...
            if args.clone().get(0).unwrap().selected != ArgTypes::Register
                && args.clone().get(0).unwrap().selected != ArgTypes::Int
            {
                panic!(
                    "Invalid type for out: {:#?} at line {}",
                    args.clone().get(0).unwrap().selected,
                    line_num + 1
                )
            }
            if args.clone().get(0).unwrap().selected == ArgTypes::Register {
                print!(
                    "{}",
                    get_register(
                        args.clone().get(0).unwrap().clone().register.unwrap(),
                        registers.clone()
                    )
                    .unwrap()
                    .i64type
            ...
            if args.clone().get(0).unwrap().selected == ArgTypes::Int {
                print!(
                    "{}",
                    args.clone().get(0).unwrap().clone().int.unwrap() as u8 as char
                )
            }
        } else if op == "sout" {
            if arg_count != 1 {
            ...
            if args.clone().get(0).unwrap().selected != ArgTypes::Register
                && args.clone().get(0).unwrap().selected != ArgTypes::String
            {
                panic!(
                    "Invalid type for sout: {:#?} at line {}",
                    args.clone().get(0).unwrap().selected,
                    line_num + 1
                )
            }
            ...
            if args.clone().get(0).unwrap().selected != ArgTypes::String {
                panic!(
                    "Invalid type {:#?} for set at line {}",
                    args.clone().get(0).unwrap().selected,
                    line_num + 1
                );
            }
            variables.insert(
                args.clone().get(0).unwrap().clone().string.unwrap(),
                stack.pop().unwrap(),
            );
        } else if op == "get" {
            if arg_count != 1 {
            ...
            stack.push(
                variables
                    .get(&args.clone().get(0).unwrap().clone().string.unwrap())
                    .unwrap()
                    .to_owned(),
            );
        }
